...
\title[Joint visualization of influenza data]{Joint visualization of seasonal influenza serology and phylogeny to inform vaccine composition}
...
Of our two prior antigenic visualizations described above, the static heatmaps addressed the most user goals except for communicating the number of measurements available for each clade that support the summary statistic of mean titer distance.
Both of these prior visualizations either obscured or hid the underlying distributions of the raw data.
...
By default, groupings appear in descending order by number of measurements.
Users can specify a custom default order for individual groups by providing a custom order in the measurements sidecar JSON file.
Users can also manually assign an order for specific groupings by filtering measurements to the corresponding grouping labels (Figure~\ref{fig:2}d).
...
Serum against A/HongKong/1985/2009 had a mean antigenic distance of 0.56 $\pm$ 1.45 units to test viruses from 158N/189K, while serum against A/Alaska/5/2010 had a mean distance of 0.48 $\pm$ 1.66 units.
...
This view also revealed that test viruses with same 144K genotype as A/HongKong/1985/2009 had lower antigenic distances (mean of -0.56 $\pm$ 0.75 units), while viruses with the ancestral 144N genotype had higher antigenic distances (1.58 $\pm$ 1.14 units).
Interestingly, measurements against A/Alaska/5/2010 showed similar distances for both test viruses with 144N (0.57 $\pm$ 1.72 units) and those with 144K (0.13 $\pm$ 1.51 units).
...
Labels appear in descending order by the number of measurements in that group (default), by the custom order defined in the measurements sidecar JSON file or in the order they have been selected by a user filter
