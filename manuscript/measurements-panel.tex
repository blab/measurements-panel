...
Test viruses of the titer data are directly linked to the viruses displayed in the phylogeny tree via their strain names to ensure that any interactions with the tree also affect the measurements displayed.
Filters applied to the tree via the date slider (Figure~\ref{fig:2}a), the data filter (Figure~\ref{fig:2}d), or clicking on tree branches to zoom into clades filters measurements to only matching test viruses.
Users can easily focus on a subset of measurements that is phylogenetically relevant and examine the measurements of test viruses in recently circulating clades.
The measurements are colored by the same coloring attribute (Figure~\ref{fig:2}b) as the phylogeny tree, adding the legend values (Figure~\ref{fig:2}c) as another dimension of nominal data to the titer measurements.
This is especially useful for applying genotypes to the titer data, allowing users to reveal potential cause and effect relationships between mutations and antigenic drifts quantified in titer measurements.

The measurements JSON can include multiple collections of titers for a single phylogeny tree and users can change the collection displayed with the collection dropdown (Figure~\ref{fig:2}e).
Users can then review different sets of data for the same phylogeny tree such as separate measurements of cell and egg passaged virus titers.
...
For vaccine selection, the most useful grouping is the reference strain (Figure~\ref{fig:2}j) to directly compare titers for individual vaccine candidates.
...
We will discuss the application of these features in detail in two case studies in the following sections.
...
\begin{figure}[h!]
  ...
  \end{center}
  \caption{
    Serological analysis of the fall 2009 (southern hemisphere) vaccine selection candidates.
    A) Phylogenetic tree summarizing the clade turnover. At the selection deadline (VCM) clade 144K (light blue) was dominant, with A/Perth/16/2009 the candidate reference sera from this clade. A/Brisbane/10/2007 was used in the previous season's vaccine.
    B) Neutralization titers against three reference sera, representing candidate vaccine strains, using data available prior to the VCM. $\log_{2}>2$ indicates inability to cover viruses from that clade.
    C) Same as (B) but including test viruses collected after the VCM, which were mainly from clade 158N/189K.
  }
  \label{fig:3}
\end{figure}
...
\begin{figure}[h!]
  ...
  \end{center}
  \caption{
    Neutralization of clade 144V viruses by clade 144V sera.
    A) Summary phylogenetic tree with clade 144V highlighted in red.
    B) Neutralization results when viewed as mean±sd show a wide range of values indicating a large amount of antigenic diversity.
    C) When viewing the individual measurements themselves, they suggest a previously hidden bimodal distribution in the measurements for the A/Shanghai/11/1987 reference strain compared to an apparently unimodal distribution for the A/Beijing/353/1989 reference strain.
    D) Coloring these measurements by RBD site HA1:135 reveals a  potential genotype-specific explanation for the two clusters seen in the A/Shanghai/11/1987 measurements and a similar bimodal distribution in the measurements for A/Beijing/353/1989 that was not as clear when coloring by clade alone.
  }
  \label{fig:4}
\end{figure}
